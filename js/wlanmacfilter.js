...
var g_wlan_multiMacfilter = null;
var g_wlan_handover = null;
var g_wlan_multi_macfilter_immdiate = "0";
...
$(document).ready(function() {
    $('#ssid1_mac_list :input').val('');
    $('#ssid2_mac_list :input').val('');
    ...
    $('#ssid1_mac_list :input').bind('input change cut paste keydown', function(e) {
    ...
    if (g_module.multi_ssid_enabled) {
            getAjaxData('api/wlan/multi-switch-settings', function($xml) {
                ...
                g_wlan_multiMacfilter = ret.response;
            },{
                sync:true
            });
        ...
        if(g_wlan_handover.Handover == '0' || g_wlan_handover.wifidataswitch == '0'){
            if(g_wlan_multiMacfilter.multissidstatus == 1){
                $('#ssid1_name').show();
                $('#ssid2').show();
                ...
                $('#ssid2_mac_list :input').bind('input change cut paste keydown', function(e) {
                    if(MACRO_KEYCODE != e.keyCode){
                        button_enable('apply', '1');
                    }
                });
...
function wlanmacfilter_multissid_initPage() {
    ...
    getAjaxData('api/wlan/multi-basic-settings', function($xml) {
        ...
        g_wlan_multi_macfilter_immdiate = g_wanlmacfilter_dataVlaue[0].macfilterimmediatework;
	    setData(ssids);
		if(($.browser.msie) && ($.browser.version == 6.0))
		{
			setTimeout(function() {
...
function multissid_postData() {
    ...
    if ("1" == g_wlan_multi_macfilter_immdiate){
        postData.WifiRestart = "0";
    }
    else{
        postData.WifiRestart = "1";
    }
